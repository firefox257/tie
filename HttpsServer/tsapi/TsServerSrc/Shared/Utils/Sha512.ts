 ...
 */
 class Sha512 {
     ...
     */
    static Long:any;
    static hash(msg:any, options:any) {
        ...
        function hexBytesToString(hexStr:any) { // convert string of hex numbers to a string of chars (eg '616263' -> 'abc').
            const str:any = hexStr.replace(' ', ''); // allow space-separated groups
            return str=='' ? '' : str.match(/.{2}/g).map((byte:any) => String.fromCharCode(parseInt(byte, 16))).join('');
        }
    }
     ...
     * @private
     */
    static ROTR(x:any, n:any) { // emulates (x >>> n) | (x << (64-n)
        if (n == 0) return x;
     ...
     * @private
     */
    static Σ0(x:any) { return Sha512.ROTR(x, 28).xor(Sha512.ROTR(x, 34)).xor(Sha512.ROTR(x, 39)); }
    static Σ1(x:any) { return Sha512.ROTR(x, 14).xor(Sha512.ROTR(x, 18)).xor(Sha512.ROTR(x, 41)); }
    static σ0(x:any) { return Sha512.ROTR(x,  1).xor(Sha512.ROTR(x,  8)).xor(x.shr(7)); }
    static σ1(x:any) { return Sha512.ROTR(x, 19).xor(Sha512.ROTR(x, 61)).xor(x.shr(6)); }
    static Ch(x:any, y:any, z:any)  { return (x.and(y)).xor(x.not().and(z)); }         // 'choice'
    static Maj(x:any, y:any, z:any) { return (x.and(y)).xor(x.and(z)).xor(y.and(z)); } // 'majority'
...
Sha512.Long = class {
    ...
    add(that:any) { // addition modulo 2^64
