...
(function()
{
        ...
        function resource(path, filepath)//obj, objc, path, filepath)
        {
            ...
            function bindGet()
            {
                return function()
                {
                    var src = $.syncfetchtext(filepath);
                    src = src.trim();
                    var b = {};
                    ...
                    if(b.comp != undefined)
                    {
                        var compname = (b.comp.split(/\r?\n/).map(d=>d.trim()).filter(d=>d != ""))[0];
                        jssrc = `
                            ${b.js}
                            $.comp("${compname}", ${compname}, "${b.css}", "${b.html}", );
                        `;
        ...
        function resourceComp(name, filepath)//obj, objc, name, filepath)
        {
